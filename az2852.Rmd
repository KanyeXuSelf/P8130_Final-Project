...
# Adjust the color scale to increase visibility
corrplot(cor(math_df_numeric), 
         type = "upper",           # Only show the upper part of the matrix
         diag = FALSE,             # Hide diagonal
         col = colorRampPalette(c("blue", "white", "red"))(200),  # Custom color scale
         is.corr = TRUE,           # Treat input as correlations
         tl.col = "black",         # Text label color
         tl.srt = 45,              # Rotate text labels
         cl.lim = c(-1, 1),        # Set limits for the color legend
         method = "color",         # Use color shading to represent values
...
significant predictors (7), same as stepwise model: 
`gender`
`ethnic_group`
`parent_educ`
`lunch_type`
...
```
interactions: ethnic group E and test prep none
```{r}
